...
🥈 SILVER-SPECIFIC OPTIMIZATIONS:
  - Higher volatility parameters tuned for Silver's increased price swings vs Gold
  - Adjusted ATR thresholds for Silver's typical volatility range (2-3x Gold volatility)
  - Modified angle scale factors for Silver price movements
...
ATR VOLATILITY FILTER (SILVER-OPTIMIZED)
-----------------------------------------
🌊 PURPOSE: Ensures trades occur during sufficient market volatility for Silver
   - SILVER typically shows 2-3x higher volatility than Gold
   - LONG: ATR range 0.000300-0.001200 with decrement filtering (-0.000100 to -0.000001)
   - SHORT: ATR range 0.000600-0.001500 with increment filtering (0.000020 to 0.000300)
   - ATR change requirement measures Silver's momentum direction
   - Pullback mode: Compares ATR from signal detection to breakout phase
   - Standard mode: Checks current ATR against minimum threshold for Silver moves

EXIT SYSTEM (SILVER-TUNED)
--------------------------
🎯 PRIMARY: ATR-based Stop Loss & Take Profit (OCA orders) - Silver Optimized
   ...
   - Silver multipliers adjusted for higher volatility and momentum characteristics
...
LONG_ATR_INCREMENT_MAX_THRESHOLD = 0.0111     # 🥈 SILVER: Increased from 1.6 (3x for Silver volatility)
...
LONG_ATR_DECREMENT_MIN_THRESHOLD = -0.004 # 🥈 SILVER: Increased from -0.00002 (3x for Silver volatility)
LONG_ATR_DECREMENT_MAX_THRESHOLD = 0 # 🥈 SILVER: Increased from -0.00001 (3x for Silver volatility)
...
class KipsStrategy(bt.Strategy):
    ...
    def _calculate_forex_position_size(self, entry_price, stop_loss_price):
        ...
        # Calculate value per tick for XAGUSD
        # For XAGUSD: 1 standard lot (5000 oz) = $50 per tick (0.01 price move)
        
        if self.p.forex_quote_currency == 'USD':
            value_per_pip_per_lot = (self.p.forex_pip_value * self.p.forex_lot_size)
        else:
            # For XAGUSD, direct USD pricing
    ...
    def _format_forex_trade_info(self, entry_price, stop_loss, take_profit, lot_size, pip_risk, position_value, margin_required):
        ...
        # Silver: $0.05 per tick for standard lot (5000 oz)
        ...
        return (f"\n--- SILVER TRADE DETAILS ({self.p.forex_instrument}) ---\n"
